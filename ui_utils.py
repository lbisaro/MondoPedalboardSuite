...
class FrequencyPlotWidget(pg.PlotWidget):
    ...
    def __init__(self, parent=None, title="", y_range=(-30, 30), **kwargs):
        ...
        # Target bands attributes and connection
        self.target_regions = []
        self.target_labels = []
        self.band_labels = []
    ...
    def set_target_ranges(self, ranges):
        """
        Dibuja las bandas verticales semi-transparentes de target en el gráfico.
        ranges: lista de diccionarios [{'name': 'BODY', 'min_freq': 100, 'max_freq': 500, 'color': '#9370DB'}, ...]
        """
        # Limpiar bandas anteriores
        ...
        for r in ranges:
            ...
            # Relleno sutil con ~8% de opacidad
            bg_color = QtGui.QColor(color.red(), color.green(), color.blue(), 20)
            bg_brush = pg.mkBrush(bg_color)
            
            # Bordes sutiles con ~25% de opacidad
            border_color = QtGui.QColor(color.red(), color.green(), color.blue(), 64)
            border_pen = pg.mkPen(color=border_color, width=1)
            
            # Los valores en el eje X logarítmico deben estar en escala log10
            val_min = np.log10(min_freq)
            val_max = np.log10(max_freq)
            ...
            # Configurar pinceles de bordes y desactivar hover
            region.lines[0].setPen(border_pen)
            region.lines[1].setPen(border_pen)
            region.setHoverBrush(pg.mkBrush(0, 0, 0, 0))
            
            self.addItem(region)
            self.target_regions.append(region)
            
            # Crear etiqueta de texto arriba
            label_color = QtGui.QColor(color.red(), color.green(), color.blue(), 180)
            ...
            y_top = y_range[1]
            y_pos = y_top - (y_top - y_range[0]) * 0.04
            
            center_log = (val_min + val_max) / 2
            text_item.setPos(center_log, y_pos)
            
            self.addItem(text_item)
            self.target_labels.append(text_item)
            self.band_labels.append((text_item, center_log))

    def update_band_labels(self):
        """Mantiene las etiquetas alineadas en la parte superior del gráfico al hacer zoom/pan."""
        if not hasattr(self, 'band_labels') or not self.band_labels:
            return
        y_range = self.plotItem.vb.viewRange()[1]
        y_top = y_range[1]
        y_bottom = y_range[0]
        y_pos = y_top - (y_top - y_bottom) * 0.04
        for label, x_pos in self.band_labels:
