...
class SlotInfo:
    ...
    FIELD_MAP_03 = {
        0x13: ("slot_type", "byte"),
        0x14: ("unknown_1_x83", ("until_marker", [0x07])),
        0x07: ("parameter", "until_end")
    }
    ...
    def id_to_names(self):
        ...
        if self.amp_effect_slot_a != b'\xff':
            ...
            try:
                ...
                readable_name_a = ["Unknown", f"Unknown Block ({beauty_str})"]
        if self.amp_effect_slot_b != b'\xff':
            ...
            try:
                ...
                readable_name_b = ["Unknown", f"Unknown Block ({beauty_str})"]
            ...
            while cursor < len(data):
                ...
                if field_type == "byte":
                    ...
                    if field_name == "info_slot_a":
                        num_params = value[2]
                        raw_params = value[7:]
                        if len(raw_params) >= 2 and raw_params[0] == 0x00 and raw_params[1] == num_params:
                            raw_params = raw_params[2:]
                        params, bytes_processed = self.read_params(num_params, raw_params)
...
class HxPreset:
    ...
    def _parse(self):
        ...
        # Ensure we have at least 5 elements in self.switch_info to avoid crashes in to_string()
        while len(self.switch_info) < 5:
            try:
                dummy = FootSwitchInfo(b'\xc0', debug=False)
                self.switch_info.append(dummy)
            except Exception:
                break
    ...
    def extract_footswitch_sections(data):
        # Find next occurrence of any "089" (e.g. 0895 or 089d)
        begin = data.find('089')
        if begin == -1:
            return []
        data = data[begin + 4:]
        
        # In older firmware, it ends with '049a'. In newer firmware, it might not have it.
        end = data.find('049')
        if end != -1:
            data = data[:end]
        ...
        # divide into switch information
        footswitch_sections_idx = []
        begin_sec = -1
        end_sec = -1

        # divide into switch information
        for peek_cursor in range(0, len(data_bytes)):
            if (data_bytes[peek_cursor] & 0xF0) == 0x90 and data_bytes[peek_cursor + 1] == 0x87:
                # begin of a new footswitch info section
                if begin_sec != -1:
                    footswitch_sections_idx.append((begin_sec, peek_cursor))
                    begin_sec = -1
                    end_sec = -1
                begin_sec = peek_cursor
            elif data_bytes[peek_cursor] == 0xc0:
                # empty slot
                if begin_sec != -1:
                    footswitch_sections_idx.append((begin_sec, peek_cursor))
                begin_sec = end_sec = peek_cursor
                footswitch_sections_idx.append((begin_sec, end_sec + 1))
                begin_sec = -1
                end_sec = -1
        if begin_sec > -1:
            footswitch_sections_idx.append((begin_sec, len(data_bytes)))

        footswitch_sections_data = []
        for section in footswitch_sections_idx:
            footswitch_sections_data.append(data_bytes[section[0]:section[1]])
    ...
    def extract_slot_sections(data):
        ...
        for peek_cursor in range(0, len(data_bytes)):
            if (data_bytes[peek_cursor] & 0xF0) == 0x80 and data_bytes[peek_cursor + 1] == 0x13:
                # begin of a new slot section
                if begin_sec != -1:
                    slot_sections_idx.append((begin_sec, peek_cursor))
                begin_sec = peek_cursor
        if begin_sec > -1:
            slot_sections_idx.append((begin_sec, len(data_bytes)))

        slot_sections_data = []
        for section in slot_sections_idx:
            slot_sections_data.append(data_bytes[section[0]:section[1]])
